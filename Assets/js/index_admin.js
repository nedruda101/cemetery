...
$(document).ready(function() {
    ...
    $('.customer-site').change(function() {
        let siteId = $(this).val();
        let customerId = $(this).data('id'); // Get the customer ID for this specific form

        // Enable the sector dropdown
        $('#customer-sector-' + customerId).prop('disabled', false);

        // Fetch sectors based on the selected site
        $.ajax({
            url: 'fetch_sectors.php', // Your PHP file to fetch sectors
            method: 'POST',
            data: { site_id: siteId },
            success: function(response) {
                $('#customer-sector-' + customerId).html(response);
            }
        });
    });
    ...
    $('.customer-sector').change(function() {
        let sector = $(this).val();
        let siteId = $('#customer-site-' + $(this).data('id')).val(); // Get the site ID
        let customerId = $(this).data('id'); // Get the customer ID for this specific form

        // Enable the block dropdown
        $('#customer-block-' + customerId).prop('disabled', false);

        // Fetch blocks based on the selected sector
        $.ajax({
            url: 'fetch_blocks.php', // Your PHP file to fetch blocks
            method: 'POST',
            data: {
                site_id: siteId,
                sector: sector
            ...
            success: function(response) {
                $('#customer-block-' + customerId).html(response);
            }
        });
    });

    // When the block dropdown changes
    $('.customer-block').change(function() {
        let blockId = $(this).val();
        let customerId = $(this).data('id'); // Get the customer ID for this specific form

        // Enable the lot dropdown
        $('#customer-lot-' + customerId).prop('disabled', false);

        // Fetch lots based on the selected block
        $.ajax({
            url: 'fetch_lots.php', // Your PHP file to fetch lots
            method: 'POST',
            data: { block_id: blockId },
            success: function(response) {
                $('#customer-lot-' + customerId).html(response);
            }
        });
    });

    // When the lot dropdown changes
    $('.customer-lot').change(function() {
        let lotId = $(this).val();
        let customerId = $(this).data('id'); // Get the customer ID for this specific form

        // Fetch lawn type based on the selected lot
        $.ajax({
            url: 'fetch_lawn_type.php', // Your PHP file to fetch lawn type
            method: 'POST',
            data: { lot_id: lotId },
            success: function(response) {
                $('#customer-lawn-type-' + customerId).val(response);
            }
        });
    });
});